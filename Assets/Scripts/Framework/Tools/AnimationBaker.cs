...
namespace Framework
{
    ...
    public class AnimationBaker : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox(GetBakeInfo(), UnityEditor.MessageType.Info);
            }
            ...
            EditorGUILayout.HelpBox(
                "使用说明：\n" +
                "1. 选择包含 SkinnedMeshRenderer �?MeshFilter 的预制体\n" +
        ...
        {
            EditorUtility.DisplayProgressBar("烘焙动画", "准备�?..", 0f);
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int frame = 0; frame < totalFrames; frame++)
                    {
                        ...
                        EditorUtility.DisplayProgressBar("烘焙动画", $"处理�?{frame + 1}/{totalFrames}", progress);
                        ...
                        for (int meshIndex = 0; meshIndex < skinnedMeshes.Length; meshIndex++)
                        {
                            ...
                            // 保存第一帧的烘焙 Mesh（使用原�?sharedMesh 的拓扑结构）
                            if (frame == 0)
                            {
                                ...
                                UnityEngine.Debug.Log($"[AnimationBaker] Mesh {meshIndex} ({skinnedMesh.gameObject.name}): 顶点�?{meshData.vertexCount}, 三角形数={meshData.bakedMesh.triangles.Length / 3}");
                            }
                            ...
                            {
                                UnityEngine.Debug.LogError($"[AnimationBaker] Mesh {meshIndex} 帧{frame} 顶点数不匹配！期�? {meshData.vertexCount}, 实际: {vertices.Length}");
                                continue;
                            }
