...
namespace PVZ
{
    ...
    public partial struct ZombieSpawnSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            ...
            _initialized = false; // 延迟到第一�?OnUpdate 从配置初始化
        }

        public void OnUpdate(ref SystemState state)
        {
            // 检查游戏状态，只在Playing时生成僵�?
            ...
            if (!_initialized)
            {
                ...
                if (SystemAPI.TryGetSingleton<LevelConfigComponent>(out var levelConfig))
                {
                    ...
                    _rowCount = levelConfig.RowCount;  // 使用关卡的行�?
                    _columnCount = levelConfig.ColumnCount; // 使用关卡的列�?
                    ...
                    configLoaded = true;
                    GameLogger.Log("ZombieSpawnSystem", $"从关卡配置初始化。生成间�?{_spawnInterval}s, 延迟={_startDelay}s, 行数={_rowCount}, 列数={_columnCount}, 格子大小=({_cellWidth}, {_cellHeight})");
                }
                // 如果没有关卡配置，尝试使用游戏全局配置
                else if (SystemAPI.TryGetSingleton<GameConfigComponent>(out var config))
                {
                    ...
                    configLoaded = true;
                    GameLogger.Log("ZombieSpawnSystem", $"从全局配置初始化。生成间�?{_spawnInterval}s, 延迟={_startDelay}s, 行数={_rowCount}");
                }
