...
namespace DG.DOTweenEditor
{
    ...
    public class DOTweenAnimationInspector : ABSAnimationInspector
    {
        ...
        static readonly Dictionary<DOTweenAnimation.AnimationType, AllowedType[]> _Tk2dAnimationTypeToComponent = new Dictionary<DOTweenAnimation.AnimationType, AllowedType[]>() {
...
#endif
#if false // TEXTMESHPRO_MARKER
        static readonly Dictionary<DOTweenAnimation.AnimationType, AllowedType[]> _TMPAnimationTypeToComponent = new Dictionary<DOTweenAnimation.AnimationType, AllowedType[]>() {
...
//                _src.animationType = (DOTweenAnimation.AnimationType)EditorGUILayout.EnumPopup(_src.animationType, EditorGUIUtils.popupButton);
                ...
                if (_src.loops > 1 || _src.loops == -1)
                    _src.loopType = (LoopType)EditorGUILayout.EnumPopup("   Loop AllowedType", _src.loopType);
        ...
        bool Validate(GameObject targetGO)
        {
            ...
            if (_Tk2dAnimationTypeToComponent.ContainsKey(_src.animationType)) {
                foreach (AllowedType t in _Tk2dAnimationTypeToComponent[_src.animationType]) {
            ...
            if (_TMPAnimationTypeToComponent.ContainsKey(_src.animationType)) {
                foreach (AllowedType t in _TMPAnimationTypeToComponent[_src.animationType]) {
                    srcTarget = targetGO.GetComponent(t);
                    if (srcTarget != null) {
                        _src.target = srcTarget;
                        _src.targetType = DOTweenAnimation.TypeToDOTargetType(t);
