...
half4 WaterFragment(WaterVertexOutput IN) : SV_Target
{
    ...
    // Detail waves
	half2 detailUV = IN.uv.xy * 0.3;
	half2 detailUV2 = IN.uv.zw * 0.3;
	half2 detailBump1 = SAMPLE_TEXTURE2D(_SurfaceMap, sampler_SurfaceMap, detailUV2).xy * 2 - 1;
	half2 detailBump2 = SAMPLE_TEXTURE2D(_SurfaceMap, sampler_SurfaceMap, detailUV).xy * 2 - 1;
	...
	reflection += _ReflectionColor.rgb * _ReflectionColor.a;

	// Refraction
	half3 refraction = Refraction(distortion, depth.x, depthMulti);
	refraction += _RefractionColor.rgb * _RefractionColor.a;

	// return half4(refraction + sss,1);
	return half4(lerp(refraction, reflection, fresnelTerm) + spec,1);
	// Do compositing
