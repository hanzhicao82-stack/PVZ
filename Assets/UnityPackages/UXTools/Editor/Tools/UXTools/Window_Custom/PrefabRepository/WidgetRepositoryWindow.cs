...
namespace ThunderFireUITool
{
    ...
    public class WidgetRepositoryWindow : EditorWindow
    {
        ...
        [MenuItem(ThunderFireUIToolConfig.Menu_WidgetLibrary, false)]
        ...
        private List<WidgetData> prefabInfoList;
        ...
        private void InitWindowData(bool issortbydict)
        {
            ...
            if(!issortbydict)
            {
                prefabInfoList.Sort((f1, f2) => f1.GUID.CompareTo(f2.GUID));
            }
        ...
        private void RefreshRightPrefabContainer(bool issortbydict=false,string type = null)
        {
            ...
            {
                fliterItems = asstesItems.Where(item => item.label == filtration).ToList();
            }
        ...
        private void ReLayoutRightContainer()
        {
            ...
            List<WidgetData> tmp1 = PrefabUtils.GetWidgetList();
        ...
        private DragAndDropVisualMode OnHierarchyGUI(int dropTargetInstanceID, HierarchyDropFlags dropMode, Transform parentForDraggedObjects, bool perform)
        {
            if (perform)
            {
                if (RightContainerDrag && LoadPrefab != null)
                {
                    ...
                    bool hasParent = obj != null;
                    //  TODO 优化，检测如果没有父节点，就去找Canvas，如果没有，就自动创建Canvas
                    ...
                    bool isUnpack = !IsPack;
                    if (dropMode.HasFlag(HierarchyDropFlags.DropUpon))
                    {
                        if (hasParent && dropTargetInstanceID != UnityEngine.SceneManagement.SceneManager.GetActiveScene().handle)
                        {
                    ...
                    currentPrefab.transform.localScale = LoadPrefab.transform.localScale;
        ...
        private void CreatePrefabWidget(GameObject obj, string path)
        {
            ToolUtils.CreatePrefabAsWidget(obj, obj.name, path, true, filtration, true);
        }
        ...
        private void DoPrefabDrag()
        {
            if (RightContainerDragIn && !RightContainerDrag)
            {
                ...
                if (dragObjList.Count == 1)
                {
                    ...
                    {
                        CreatePrefabWidget(dragObjList[0], path);
